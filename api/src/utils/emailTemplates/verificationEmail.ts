...
export function getVerificationEmail({ username, link }: { username: string; link: string }) {
  ...
  const safeHtmlLink = sanitizeHtml(link);
  
  const text = `Hello ${safeUsername},\n\nPlease verify your email address by clicking the link below:\n${safeLink}\n\nIf you did not create an account, you can ignore this email.`;
  const html = `
    <div style="font-family: sans-serif; color: #222;">
      <h2>Hello ${safeHtmlUsername},</h2>
      <p>Please verify your email address by clicking the button below:</p>
      <p style="margin: 32px 0;">
        <a href="${safeHtmlLink}" style="background: #2563eb; color: #fff; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: bold;">Verify Email</a>
      </p>
      <p>If the button doesn't work, copy and paste this link into your browser:</p>
      <p><a href="${safeHtmlLink}">${safeHtmlLink}</a></p>
