...
export default function UploadContent() {
  ...
  const sanitizeToWindowsName = (input: string): string => {
    ...
    // Remove .torrent suffix for display
    name = name.replace(/\.torrent$/i, '');
    // Replace invalid Windows filename characters with underscore
    name = name.replace(/[<>:"/\\|?*\x00-\x1F]/g, '_');
    // Replace emojis (extended pictographic) and variation selectors/ZWJ with underscore
    try {
  ...
  const handleFileSelect = useCallback((file: File) => {
    if (file.size > MAX_FILE_SIZE) {
      toast.error('Torrent file is too large (max 10MB)');
      return;
    }
  ...
  const handleDrop = useCallback((e: React.DragEvent) => { e.preventDefault(); setIsDragOver(false); const f = Array.from(e.dataTransfer.files).find(x => x.name.endsWith('.torrent')); if (f) handleFileSelect(f); }, [handleFileSelect]);
  ...
  const handleNfoSelect = useCallback((file: File) => {
    if (file.size > MAX_FILE_SIZE) {
      toast.error('NFO file is too large (max 10MB)');
      return;
    }
    setUploadedNfo(file);
  }, []);
  ...
  const onSubmit = useCallback(async (data: UploadFormData) => {
    if (!uploadedFile) {
      toast.error('Selecciona un archivo .torrent');
      return;
    }
    setIsUploading(true);
    try {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <ImageUploadArea
            ...
            onDrop={(e) => { e.preventDefault(); setIsImageDragOver(false); const f = Array.from(e.dataTransfer.files).find(x => x.type.startsWith('image/')); if (f) handleImageSelect(f); }}
        ...
        <div className="lg:col-span-2">
          ...
          <NfoUploadArea
            ...
            onDragOver={(e) => { e.preventDefault(); setIsNfoDragOver(true); }}
            onDragLeave={(e) => { e.preventDefault(); setIsNfoDragOver(false); }}
            onDrop={(e) => { e.preventDefault(); setIsNfoDragOver(false); const f = Array.from(e.dataTransfer.files).find(x => x.name.toLowerCase().endsWith('.nfo')); if (f) handleNfoSelect(f); }}
            isDragOver={isNfoDragOver}
            loading={loading}
          />
        </div>
        ...
        <div className="lg:col-span-2">
          <UploadOptions
            ...
            onAnonymousChange={(v) => setValue('anonymous', v, { shouldValidate: true, shouldDirty: true })}
            onFreeleechChange={(v) => setValue('freeleech', v, { shouldValidate: true, shouldDirty: true })}
            loading={loading}
          />
        </div>
        ...
        <UploadActions
          ...
          disabledReasons={(() => {
            ...
            // Añadir mensajes de validación de Zod/RHF si existen
            const zodMsgs = Array.from(new Set(Object.values(errors).map((e: any) => e?.message).filter(Boolean)));
            reasons.push(...zodMsgs as string[]);
            return reasons;
          })()}
          notes={(uploadedFile ? ['Recuerda: el .torrent debe estar marcado como privado y contener tu passkey en la URL de announce.'] : [])}
          loading={loading}
        />
      </form>
